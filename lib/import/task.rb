...
module Import
    ...
    class Task
      ...
      def execute
        ...
        elsif target_adapter == "xlsx"
          return_value = import_xlsx
      ...
      # ── xlsx adapter ──────────────────────────────────────────────────────────
      # Reads an .xlsx file from import_folder using the Roo gem.
      # Mirrors import_csv but handles Roo's native typed values (Date, DateTime,
      # Numeric) so no unnecessary string round-trips are needed.
      # Optional config key: sheet_name — defaults to the first sheet.

      def xlsx_file_path
        @xlsx_file_path ||= File.join(@main_driver.import_folder, import_file_name)
      end

      def import_xlsx
        xlsx       = Roo::Spreadsheet.open(xlsx_file_path)
        sheet_name = task_config["sheet_name"] || xlsx.sheets.first
        sheet      = xlsx.sheet(sheet_name)

        # Normalise header row to snake_case attribute names
        raw_headers = sheet.row(1).map { |h| h.to_s.strip }
        headers     = raw_headers.map  { |h| Util.column_to_attribute(h) }
        headers.each do |col|
          if columns_hash[col].blank?
            headers[headers.index(col)] = col.split(/\_+/).first
          end
        end
        ...
        unmatched = headers.select { |col| columns_hash[col].blank? }
        log "!!WARNING!!: These columns are not processed: [#{unmatched.join(", ")}]" if unmatched.present?
        ...
        ActiveRecord::Base.transaction do
          ...
          (2..sheet.last_row).each do |row_idx|
            ...
            row_hash  = headers.each_with_index.each_with_object({}) { |(col, i), h| h[col] = raw_row[i] }
            row_error = false
            attributes = {}

            headers.each do |col|
              next if columns_hash[col].blank?

              val      = row_hash[col]
              col_type = columns_hash[col].type

              # Treat blank/whitespace strings as nil
              val = nil if val.respond_to?(:strip) && val.strip.empty?

              case col_type
              when :integer
                if val.is_a?(Numeric)
                  val = val.to_i
                elsif !Util.valid_integer?(val.to_s)
                  log "Invalid integer [#{val}] in column: #{col} row: #{row_hash}"
                  n_errors += 1; row_error = true; next
                end
              when :datetime, :date
                if val.is_a?(DateTime) || val.is_a?(Time) || val.is_a?(Date)
                  val = val.to_datetime
                elsif !Util.valid_datetime?(val.to_s)
                  log "Invalid datetime [#{val}] in column: #{col} row: #{row_hash}"
                  n_errors += 1; row_error = true; next
                else
                  val = Util.parse_datetime(val.to_s)
                end
              when :string
                val = val.to_s unless val.nil?
                unless val.nil?
                  limit = columns_hash[col].sql_type_metadata.limit
                  if limit && val.length > limit
                    log "Length of value [#{val}] exceeds maximum for column #{col} row: #{row_hash}"
                    n_errors += 1; row_error = true; next
                  end
                end
              when :json
            ...
            if has_checksum_index?
              checksum_cols = checksum_index.map { |ci| attributes.fetch(ci, nil) }
              attributes.merge!(checksum_index: Digest::MD5.hexdigest(checksum_cols.join))
            end

            attributes.stringify_keys!

            records << (has_ignored_columns ? attributes : class_name.new(attributes))

            if records.size >= batch_size
              n_errors += has_ignored_columns \
                ? import_records_with_ignored_columns(class_name, records) \
